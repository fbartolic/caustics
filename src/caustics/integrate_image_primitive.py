...
def _integrate_image_binary_translation(
    ...
    """
    The compilation the primitive to XLA.
...
def integrate_image_binary_grad(
    ...
    return _integrate_image_grad_prim.bind(
        rmin,
        rmax,
        theta_min,
        theta_max,
...
def _integrate_image_binary_grad_translation(
    ...
    """
    Translation rule for the C++ function which computes the gradient using 
    Enzyme.
    """
    op_name = platform.encode() + b"_integrate_image_grad"

    # Output shapes
    dims_output = (13,)
    dtype = dtypes.canonicalize_dtype(np.float64)
    shape_output = xla_client.Shape.array_shape(
        dtype, dims_output, tuple(range(len(dims_output) - 1, -1, -1))
    )
...
def _integrate_image_binary_jvp(args, tangents, **kwargs):
    ...
    primal_out = out[0] 
    tangent_out = jnp.sum(
        out[1:]*jnp.array([zero_tangent(tan) for tan in tangents])
        )
